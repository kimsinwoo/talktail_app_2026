...
export function DeviceManagementScreen() {
  ...
  const {initialMode, returnToMonitoring} = route.params || {};
  ...
  const [hubDevicesByHub, setHubDevicesByHub] = useState<
  ...
  >({});
  const [selectedMacsByHub, setSelectedMacsByHub] = useState<
    Record<string, Record<string, boolean>>
  >({});
  const [isSearchingByHub, setIsSearchingByHub] = useState<
    Record<string, boolean>
  >({});
  // ✅ 찾은 디바이스 목록 (등록되지 않은 디바이스)
  const [foundDevicesByHub, setFoundDevicesByHub] = useState<
    Record<string, string[]>
  >({});
  // ✅ 찾은 디바이스 선택 모달 표시 여부
  const [showFoundDevicesModal, setShowFoundDevicesModal] = useState<
    Record<string, boolean>
  >({});
    ...
    Promise.resolve().then(async () => {
      try {
        await Promise.allSettled(
          globalHubs.map(h => refreshHubDevices(h.address)),
        );
      } catch {
        // ignore
      }
  ...
  }, [createOrUpdateDeviceInDb, refreshHubDevices]);
  ...
  useEffect(() => {
    ...
    hubBleService.setOnDisconnectCallback?.(() => {
      console.log(
        '[DeviceManagementScreen] 🔄 BLE 연결 해제 감지, 허브 목록 업데이트',
      );
      refreshHubs().catch(() => {});
    });
  ...
  // 찾은 디바이스 목록을 저장하고 모달 표시
  useEffect(() => {
    const off = hubSocketService.on('CONNECTED_DEVICES', async (payload: any) => {
      console.log('[DeviceManagementScreen] CONNECTED_DEVICES 이벤트 수신:', payload);
      const hubAddress = String(
        payload?.hubAddress || payload?.hubId || payload?.hub_address || '',
      );
      if (!hubAddress) {
        console.log('[DeviceManagementScreen] hubAddress 없음, 무시');
        return;
      }
      ...
      console.log('[DeviceManagementScreen] 최신 연결된 디바이스:', latestDevices);
      
      if (latestDevices.length === 0) {
        console.log('[DeviceManagementScreen] 연결된 디바이스 없음');
        setIsSearchingByHub(prev => ({ ...prev, [hubAddress]: false }));
        return;
      }
      ...
      try {
        ...
        const registeredDevices = (res?.data || []).map((d: any) => d.address);
        console.log('[DeviceManagementScreen] 등록된 디바이스 (API):', registeredDevices);
        
        // ✅ 등록되지 않은 디바이스만 필터링 (새로 찾은 디바이스)
        const foundDevices = latestDevices.filter(
          mac => !registeredDevices.includes(mac),
        );
        console.log('[DeviceManagementScreen] 새로 찾은 디바이스:', foundDevices);
        
        if (foundDevices.length > 0) {
          console.log('[DeviceManagementScreen] 모달 표시:', foundDevices.length, '개 디바이스');
          // ✅ 찾은 디바이스 목록 저장
          ...
          setShowFoundDevicesModal(prev => {
            ...
            console.log('[DeviceManagementScreen] 모달 상태 업데이트:', newState);
            return newState;
          });
          // ✅ 디바이스 이름 초기화 (draft)
          ensureDraftsForHubMacs(hubAddress, foundDevices);
          // ✅ 선택 상태 초기화
          setSelectedMacsByHub(prev => ({
            ...prev,
            [hubAddress]: {},
          ...
          console.log('[DeviceManagementScreen] 새로운 디바이스 없음 (모두 등록됨)');
          Toast.show({
            type: 'info',
        ...
        console.error('[DeviceManagementScreen] 등록된 디바이스 조회 실패:', error);
  ...
  useEffect(() => {
    ...
    const stops = hubs.map(h =>
      hubSocketService.startHubPolling(h.address, {
        intervalMs: 60000,
        timeoutMs: 10000,
      }),
    );
  ...
  const connectHub = async (candidate: HubCandidate) => {
    ...
    fetch('http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1', {
      ...
      body: JSON.stringify({
        location: 'DeviceManagementScreen.tsx:515',
        message: 'connectHub entry',
    ...
    try {
      ...
      // #region agent log
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:521',
            message: 'hubBleService.connect before',
            data: { candidateId: candidate.id },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
      ...
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          ...
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:521',
            message: 'hubBleService.connect after',
            data: { candidateId: candidate.id, success: true },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
            hypothesisId: 'H6',
      ...
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:522',
            message: 'hubBleService.startNotifications before',
            data: { candidateId: candidate.id },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
      ...
      await hubBleService.startNotifications(candidate.id, (line: string) => {
        ...
        if (lower === 'wifi connected success') {
          ...
          if (!isProvisionDone) {
            ...
            const macLike = /^[0-9a-f]{2}(:[0-9a-f]{2}){5}$/i.test(
              candidate.id,
            );
            if (macLike) {
      ...
      hubStatusStore
        .getState()
        .setHubStatus(candidate.id.toLowerCase(), 'online');
      ...
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:574',
            message: 'setHubStep wifi',
            data: { candidateId: candidate.id },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
      ...
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:576',
            message: 'connectHub error',
            data: { candidateId: candidate.id, error: String(e?.message || e) },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
            hypothesisId: 'H6',
  ...
  const registerHubToBackend = async (hubId: string, name?: string) => {
    try {
      ...
      const hubNameToUse =
        (name || hubName || 'Tailing Hub').trim() || 'Tailing Hub';
      const res = await apiService.postRaw<{
        success: boolean;
        message?: string;
        data?: any;
      }>('/hub', {
  ...
  const sendWifiConfigToHub = useCallback(async () => {
    ...
    fetch('http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'DeviceManagementScreen.tsx:649',
        message: 'sendWifiConfigToHub entry',
        data: { hasSelectedHub: !!selectedHub, ssid: ssid.trim() },
        timestamp: Date.now(),
        sessionId: 'debug-session',
        runId: 'run1',
        hypothesisId: 'H3',
    ...
    try {
      ...
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:684',
            message: 'hubBleService.sendWifiConfig before',
            data: { peripheralId: selectedHub.id, ssid: trimmedSsid },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
            hypothesisId: 'H3',
      ...
      // #endregion
      await hubBleService.sendWifiConfig(
        selectedHub.id,
        trimmedSsid,
        password || '',
        userEmail,
      ...
      fetch(
        'http://127.0.0.1:7244/ingest/3eff9cd6-dca3-41a1-a9e7-4063579704a1',
        {
          ...
          body: JSON.stringify({
            location: 'DeviceManagementScreen.tsx:684',
            message: 'hubBleService.sendWifiConfig after',
            data: { peripheralId: selectedHub.id, success: true },
            timestamp: Date.now(),
            sessionId: 'debug-session',
            runId: 'run1',
            hypothesisId: 'H3',
      ...
      const off = hubSocketService.on('MQTT_READY', async (p: unknown) => {
        ...
        if (
          provisionStartedAt &&
          Date.now() - provisionStartedAt > 2 * 60 * 1000
        )
          return;
      ...
      mqttReadyTimeoutRef.current = setTimeout(() => {
        ...
        Toast.show({
          ...
          text1: '허브 등록 확인 실패',
          text2:
            'MQTT_READY를 받지 못했습니다. Wi‑Fi 정보가 맞는지 확인 후 다시 시도해주세요.',
          position: 'bottom',
        });
      }, 60000);
    } catch (e: any) {
              ...
              <View style={styles.hubCardWrap}>
                ...
                <View style={styles.hubContentBox}>
                  ...
                  <View style={styles.hubStatusRow}>
                    ...
                    <Text style={styles.hubStatusValue}>
                      {getHubStatusLabel(h.address, h.updatedAt)}
                    </Text>
                  </View>
                </View>
                ...
                <View style={styles.hubDevicesBox}>
                    ...
                    >
                      ...
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          {
                            backgroundColor: isSearchingByHub[h.address]
                              ? '#E5E7EB'
                              : '#E7F5F4',
                          },
                        ]}
                      ...
                      >
                        <Text
                          style={[
                            ...
                            {
                              color: isSearchingByHub[h.address]
                                ? '#9CA3AF'
                                : '#2E8B7E',
                            },
                          ]}
                        >
                          {isSearchingByHub[h.address]
                            ? '검색 중...'
                            : '전체 연결'}
                        </Text>
                      </TouchableOpacity>
                  ...
                  {(() => {
                    ...
                    if (list.length === 0) {
                      return (
                        <Text style={[styles.hubDeviceHint, { marginTop: 8 }]}>
                          등록된 디바이스 없음
                        </Text>
                      );
                    }
                        ...
                        {list.map(d => (
                          <View key={d.address} style={styles.deviceRowBox}>
                            <View style={{ flex: 1 }}>
                              ...
                              <Text style={styles.deviceRowMac}>
                                {d.address}
                              </Text>
                            </View>
                            <View style={styles.deviceRowRight}>
                              <View style={styles.deviceStatusPill}>
                                <Text style={styles.deviceStatusPillText}>
                                  {isDeviceOnlineByHub(h.address, d.address)
                                    ? '온라인'
                                    : getConnectionStatusLabel(d.updatedAt)}
                                </Text>
                              </View>
                              <TouchableOpacity
                                style={[
                              ...
                              <TouchableOpacity
                                ...
                                onPress={() =>
                                  deleteDevice(h.address, d.address)
                                }
                                activeOpacity={0.85}
                              >
                                <Text style={styles.deleteMiniButtonText}>
                                  삭제
                                </Text>
                              </TouchableOpacity>
                            </View>
        ...
        >
          <View style={styles.topHeader}>
            ...
            <Text style={styles.topHeaderSub}>
              BLE로 허브를 연결하고 Wi-Fi를 설정합니다
            </Text>
          </View>

          <View style={styles.section}>
            <View style={styles.stepperWrap}>
              {['허브 찾기', 'Wi‑Fi 설정', '연결 확인', '완료'].map(
                (t, idx) => (
                  <View key={t} style={styles.stepperItem}>
                    <View
                      style={[
                        styles.stepperDot,
                        idx <= stepIndex ? styles.stepperDotActive : null,
                      ]}
                    >
                      <Text
                        style={[
                          styles.stepperDotText,
                          idx <= stepIndex ? styles.stepperDotTextActive : null,
                        ]}
                      >
                        {idx + 1}
                      </Text>
                    ...
                    <Text
                      style={[
                        styles.stepperLabel,
                        idx <= stepIndex ? styles.stepperLabelActive : null,
                      ]}
                    >
                      {t}
                    </Text>
            ...
            {hubStep === 'scan' && (
              <View style={styles.card}>
                ...
                <Text style={styles.cardSubtle}>
                  이름이 “ESP32_S3”(또는 “Tailing_HUB”)인 BLE 장치를 찾습니다.
                </Text>
                  ...
                  {hubCandidates.length === 0 ? (
                    ...
                    hubCandidates.map(c =>
                      (() => {
                        ...
                        const isDimmed =
                          hubConnectingId !== null && hubConnectingId !== c.id;
                        return (
                          <TouchableOpacity
                            key={c.id}
                            style={[
                          ...
                          >
                            ...
                            {isConnecting ? (
                              ...
                              <Text
                                style={[
                                  styles.scanCta,
                                  isDimmed ? styles.scanCtaDisabled : null,
                                ]}
                              >
                                연결
                              </Text>
 ...
 <></>
            ...
            {hubStep === 'waiting' && (
              <View style={styles.card}>
                ...
                <Text style={styles.cardSubtle}>
                  서버에서 MQTT_READY 이벤트를 기다립니다. (허브가 Wi-Fi 연결 및
                  MQTT 연결 완료 시 전송)
                </Text>
                {!!debugText && (
                  <Text style={[styles.cardSubtle, { marginTop: 10 }]}>
            ...
            {hubStep === 'done' && (
              <View style={styles.card}>
                ...
                <Text style={styles.cardSubtle}>
                  허브가 등록되었습니다. 이제 허브에 디바이스를 연결할 수
                  있습니다.
                </Text>
                {!!debugText && (
                  <Text style={[styles.cardSubtle, { marginTop: 10 }]}>
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.card}>
            ...
            <Text style={styles.cardSubtle}>
              허브와 무관하게 휴대폰 ↔ 디바이스를 직접 연결합니다.
            </Text>
            <TouchableOpacity
              style={[
                styles.primaryButton,
                { marginTop: 12, backgroundColor: '#2E8B7E' },
              ]}
